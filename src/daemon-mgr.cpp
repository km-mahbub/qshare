...
void DaemonManager::restartSeafileDaemon()
{
    ...
    qWarning("Trying to restart qShare daemon");
    startSeafileDaemon();
}
...
{
    qDebug("qShare daemon is now running, checking if the service is ready");
...
void DaemonManager::checkDaemonReady()
{
    ...
    if (++maxcheck > kMaxDaemonReadyCheck) {
        qWarning("qShare rpc is not ready after %d retry, abort", maxcheck);
...
{
    qWarning("qShare daemon process %s with code %d ",
...
void DaemonManager::scheduleRestartDaemon()
{
    ...
    if (++restart_retried_ >= max_retry) {
        qWarning("reaching max tries of restarting qShare daemon, aborting");
