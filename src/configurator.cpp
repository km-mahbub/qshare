...
void Configurator::initSeafile()
{
    ...
    if (!preconfigure_dir.isEmpty()) {
        ...
        QString seafile_dir = dir.absoluteFilePath("qShare/.qShare-data");
...
{
    // Write seafile dir to <ccnet dir>/qShare.ini
    QFile seafile_ini(QDir(ccnet_dir_).filePath("qShare.ini"));
...
void Configurator::validateExistingConfig()
{
    ...
    QDir d(seafile_dir_);
#if !defined(Q_OS_WIN32)
    QString old_client_wt = d.filePath("../qShare/");
...
void Configurator::installCustomUrlHandler()
{
    ...
    QString classes_seafile = "Software\\Classes\\qShare";
